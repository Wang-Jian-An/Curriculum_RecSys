...
class Transformer_RecSys_Model(nn.Module):
    ...
    def forward(self, topicData, contentData):
        ...
        print(topicData.size(), contentData.size())
        ...
        topicData = topicData["pooler_output"]
        contentData = contentData["pooler_output"]
        interactionData = torch.cat([topicData, contentData], dim = -1)
        return torch.sigmoid(self.decoder(interactionData))[:, -1]