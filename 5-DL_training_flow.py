...
def generate_three_obj_for_train_vali_test_data(data):
    ...
    return [
        data.query("DataID == 1 and Split == @oneSplit") for oneSplit in ["train", "vali", "test"]
    ]

def generate_TrainDataLoader(oneData: pd.DataFrame, batch_size = 128):

    print(rawTopics[oneData["Topic"].tolist()[0]])
    topicInputIdsAttentionMask = [rawTopics[oneTopic] for oneTopic in oneData["Topic"].tolist()]
    print(topicInputIdsAttentionMask[0])
    contentInputIdsAttentionMask = [rawContent[oneContent] for oneContent in oneData["Content"].tolist()]
...
if __name__ == "__main__":
    ...
    numContent = rawContent.shape[0]

    # Combine input_ids and attention mask to one list
    rawTopics["input_ids_attention_mask"] = rawTopics.copy().apply(lambda x: [list(eval(x["input_ids"])), list(eval(x["attention_mask"]))], axis = 1)
    rawContent["input_ids_attention_mask"] = rawContent.copy().apply(lambda x: [list(eval(x["input_ids"])), list(eval(x["attention_mask"]))], axis = 1)
    ...
    del rawData
    gc.collect()
    print(trainData.shape, valiData.shape, testData.shape)

    # Generate DataLoader
    trainDataLoader = generate_TrainDataLoader(oneData = trainData, batch_size = 128)
    valiDataLoader, testDataLoader = [
        generate_PredictionDataLoader(oneData = oneData, batch_size = numContent) for oneData in [valiData, testData]
    ]

    # Model Training
    model = Transformer_RecSys_Model(transformer_model_name = "bert-base-uncased").to(device)
    loss_func = nn.BCELoss()
    optimizer = torch.optim.Adam(model.parameters(), lr = 1e-3) 
    trainResult = DL_model_training()
